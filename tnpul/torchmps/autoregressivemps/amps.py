...
class AMPS(nn.Module):
    def __init__(self, n=20, bond_dim=5, std=1e-8):
        ...
        self.tensors = nn.Parameter(
            std * torch.randn(n, n, bond_dim, bond_dim, 2))

        # Initialize bias matrix, which is a (D, D, 2) identity matrix
        self.register_buffer('bias_mat', torch.stack(
            [torch.eye(bond_dim), torch.eye(bond_dim)], dim=2))
    ...
    def forward(self, data):
        ...
        logx_hat[:, 0, :] = F.log_softmax(
            (self.tensors[0, 0, :, :, :] + self.bias_mat)[0, 0, :], dim=0)

        mats = torch.einsum(
            'nlri,bi->nblr', self.tensors[:, 0, :, :, :] + self.bias_mat, embedded_data[:, 0, :])
        ...
        for idx in range(1, self.n):
            ...
            left_vec = torch.einsum(
                'nblr,nbrk->nblk', left_vec, mats)  # (n, bs, 1, D)
    ...
    def sample(self, bs, random_start=False):
        ...
        if random_start:
            samples[:, 0] = torch.randint(
                2, size=(bs, ), dtype=torch.float, device=device)
        else:
            samples[:, 0] = 0.
        ...
        for idx in range(self.n - 1):
            if idx == 0:
                ...
                embedded_data = torch.stack(
                    [samples[:, 0], 1.0 - samples[:, 0]], dim=1)  # (bs, 2)
                mats = torch.einsum(
                    'nlri,bi->nblr', self.tensors[:, 0, :, :, :] + self.bias_mat, embedded_data)
                left_vec = mats[:, :, 0, :].unsqueeze(2)  # (n, bs, 1, D)
                logits = torch.einsum('nblr, nri->nbli', left_vec,
                                      (self.tensors[:, 1, :, :, :] + self.bias_mat)[:, :, 0, :]).squeeze(2)
                samples[:, 1] = torch.bernoulli(
                    torch.softmax(logits[idx + 1, :, :], dim=1)[:, 0])
            else:
                # then sample s_3 from  p(s_3 | s_1, s_2) and so on
                embedded_data = torch.stack(
                    [samples[:, idx], 1.0 - samples[:, idx]], dim=1)  # (bs, 2)
                mats = torch.einsum(
                    'nlri,bi->nblr', self.tensors[:, idx, :, :, :] + self.bias_mat, embedded_data)
                #                 left_vec = torch.bmm(left_vec, mats)  # (bs, 1, D)
                left_vec = torch.einsum(
                    'nblr,nbrk->nblk', left_vec, mats)  # (n, bs, 1, D)
                logits = torch.einsum('nblr,nri->nbli', left_vec,
                                      (self.tensors[:, idx + 1, :, :, :] + self.bias_mat)[:, :, 0, :]).squeeze(2)
                samples[:, idx + 1] = torch.bernoulli(
                    torch.softmax(logits[idx + 1, :, :], dim=1)[:, 0])
...
class AMPSShare(nn.Module):
    def __init__(self, n=784, bond_dim=10, std=1e-8):
        ...
        self.tensors = nn.Parameter(
            std * torch.randn(n, bond_dim, bond_dim, 2))
    ...
    def forward(self, data):
        ...
        logx_hat = torch.zeros_like(embedded_data)
        logx_hat[:, 0, :] = F.log_softmax(
            (self.tensors[0, :, :, :] + self.bias_mat)[0, 0, :], dim=0)

        mats = torch.einsum(
            'lri,bi->blr', self.tensors[0, :, :, :] + self.bias_mat, embedded_data[:, 0, :])
        ...
        for idx in range(1, self.n):
            ...
            mats = torch.einsum(
                'lri,bi->blr', self.tensors[idx, :, :, :] + self.bias_mat, embedded_data[:, idx, :])
            left_vec = torch.bmm(left_vec, mats)  # (bs, 1, D)
        ...
        for idx in range(self.n - 1):
            if idx == 0:
                ...
                embedded_data = torch.stack(
                    [samples[:, 0], 1.0 - samples[:, 0]], dim=1)  # (bs, 2)
                mats = torch.einsum(
                    'lri,bi->blr', self.tensors[0, :, :, :] + self.bias_mat, embedded_data)
                left_vec = mats[:, 0, :].unsqueeze(1)  # (bs, 1, D)
                logits = torch.einsum('blr, ri->bli', left_vec,
                ...
                embedded_data = torch.stack(
                    [samples[:, idx], 1.0 - samples[:, idx]], dim=1)  # (bs, 2)
                mats = torch.einsum(
                    'lri,bi->blr', self.tensors[idx, :, :, :] + self.bias_mat, embedded_data)
                left_vec = torch.bmm(left_vec, mats)  # (bs, 1, D)
                logits = torch.einsum('blr, ri->bli', left_vec,
                                      (self.tensors[idx + 1, :, :, :] + self.bias_mat)[:, 0, :]).squeeze(1)
                samples[:, idx +
                        1] = torch.bernoulli(torch.softmax(logits, dim=1)[:, 0])
